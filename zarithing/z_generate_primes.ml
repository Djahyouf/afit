...
let eratosthenes n =
  if lt n zero
  then
    invalid_arg "negative limit ? wierd"
  else

    let rec remove n list =
      match list with
      | [] -> []
      | e::l when equal (erem e n) zero && not (equal e n)
        -> remove n l
      | e::l -> e :: remove n l
    in
    ...
    let rec _eras i era_li =
      ...
      then
        ...
        _eras (succ i) (remove i era_li)
    in _eras (succ(succ one)) (init_eratosthenes n)
;;
...
let double_primes limit isprime =
  let rec _double list =
    match list with
    | [] -> []
    | e::l when isprime e && isprime (mul (succ one) (succ e))
      -> (e, (mul (succ one) (succ e))) :: _double l
    | e::l -> _double l
